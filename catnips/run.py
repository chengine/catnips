...
catnips_configs = {
    ...
    'dt': 2e-2,                 # depth of aux/ref particle
    'Vmax': 1e-6,               # interpenetration volume
...
# catnips.save_purr('./catnips_data/purr/', save_property=True)
...
# r = 0.965/2
# dz = 0.
# center = np.array([0.03, -0.002, -0.043])
...
for (start, end) in zip(x0, xf):
    ...
    try:
        traj = planner.optimize_b_spline(As, Bs, start, end, derivatives=None)
