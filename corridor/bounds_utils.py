...
# def get_bounds(cell_sizes, traj, local=False, occupied=None, feasible=None, dilation=0.):
    
#     if local is True:
#         # Return only the sequence of cells containing the trajectory, plus 
#         # some dilation
#         bounds = get_local_box(cell_sizes, traj, dilation=dilation)
#     else:
...
#         # resulting erosion.
#         eroded_occupy = scipy.ndimage.binary_erosion(occupied)
#         surface_voxels = np.logical_and(occupied, ~eroded_occupy)
#         surface_pts = torch.tensor(feasible[surface_voxels])

#         kd = scipy.spatial.KDTree(surface_pts.cpu().numpy())

#         bounds = []
#         r_default = 0.2
#         delta = torch.tensor([lx ,ly, lz, lx, ly, lz])
#         for traj_pts in traj:
#             midpt = (traj_pts[0] + traj_pts[1])/2
#             dist = np.linalg.norm(traj_pts[0] - traj_pts[1])
#             if dist > r_default:
#                 r = dist
#             else:
#                 r = r_default

#             near_ind = kd.query_ball_point(midpt, r=r, workers=-1)
#             near_pts = surface_pts[near_ind]
            
#             minimal = torch.tensor(min_bounding_box(traj_pts, lx, ly, lz, dilation=dilation))
#             offset = np.array([r/(2*.866), r/(2*.866), r/(2*.866)])
#             maximal = torch.tensor(np.array([midpt + offset, midpt - offset])).T
...
#     bounds = bounds*torch.tensor([1., 1., 1., -1., -1., -1.])

#     maximal_bounds = maximal.T.reshape(-1)
#     maximal_bounds = maximal_bounds * torch.tensor([1., 1., 1., -1., -1., -1.])
...
# def min_bounding_box(sub_path, lx, ly, lz, dilation=0.):
#     init_cell = sub_path[0]
#     end_cell = sub_path[-1]
