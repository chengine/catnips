...
def make_kernel(robot_lims, grid, N):
...
def generate_avoid_zone(grid, kernel, get_density, discretization=100, density_factor=1., sigma=0.05):
    ...
    with torch.no_grad():
        ...
        dense = density_factor*get_density(query_pts)
        dense = dense.view(pts.shape[:-1])
    ...
    # Splitting based on vertices (x, y, z) in ([0 1], [0 1], [0 1])
    x0y0z0 = dense[:-1, :-1, :-1]   #[0, 0, 0]
    x1y0z0 = dense[1:, :-1, :-1]    #[1, 0, 0]
    x0y1z0 = dense[:-1, 1:, :-1]    #[0, 1, 0]
    x0y0z1 = dense[:-1, :-1, 1:]    #[0, 0, 1]
    x1y1z0 = dense[1:, 1:, :-1]     #[1, 1, 0]
    x0y1z1 = dense[:-1, 1:, 1:]     #[0, 1, 1]
    x1y0z1 = dense[1:, :-1, 1:]     #[1, 0, 1]
    x1y1z1 = dense[1:, 1:, 1:]      #[1, 1, 1]
    ...
    occupancy = feats.reshape((1, 8))@coeffs
    occupancy = occupancy.reshape(densities.shape[1:])
    
    #The size of each dimension depends on the relative size between the resolution of the density field and the agent
    avoid_zone = F.conv3d(occupancy[None, None, ...], kernel[None, None, ...]).squeeze()

    avoid_zone = 1 - torch.exp(-avoid_zone)
    safe_zone = (avoid_zone <= sigma)

    # if dilation is not None:
    #     coll_zones = (safe_zone == False)
    #     f = torch.nn.MaxPool3d(dilation*2 + 1, stride=1, padding=dilation)
    #     coll_zones = f(coll_zones[None, None,...].to(torch.float32))
    #     coll_zones = (coll_zones).squeeze().to(bool)
    #     safe_zone = (coll_zones == False)
            
    # x_off, y_off, z_off = kernel[0]//2 + 1, kernel[1]//2 + 1, kernel[2]//2 + 1
    # feasible_pts = pts[x_off:-x_off, y_off:-y_off, z_off:-z_off]
    ...
    return safe_zone.cpu().numpy(), trunc_center_pts.cpu().numpy()